...
classes_and_tweets = [{'class': c, 'tweet': t} for c, t in zip(similar_classes, similar_tweets)]
results = []
for data in classes_and_tweets:
    formatted_prompt = prompt.format(data['tweet'])
    result = classifier(formatted_prompt, [query], hypothesis_template="This tweet is about {}.")
    results.append({'class': data['class'], 'score': result['scores'][0]})

print("Zero-shot classification results for the query tweet:")
for result in results:
    print(f"Class: {result['class']}, Score: {result['score']:.4f}")