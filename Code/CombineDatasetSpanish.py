...
csv_path = os.path.join(os.getcwd(), 'Dataset', 'Twitter_Spanish','Partitions', 'data_split_5FCV_esp.csv')
...
for index, row in train_df.iterrows():
    class_folder = os.path.join(data_path, 'Twitter_Spanish', 'Timelines', 'Spanish', row['class'])
...
for index, row in test_df.iterrows():
    class_folder_2 = os.path.join(data_path, 'Twitter_Spanish', 'Timelines', 'Spanish', row['class'])
...
merged_train_data.to_csv(os.path.join(data_path,'train_data_esp.csv'), index=False)
merged_test_data.to_csv(os.path.join(data_path, 'test_data_esp.csv'), index=False)
