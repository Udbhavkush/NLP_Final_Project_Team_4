...
plt.title('Year-wise Distribution of English Tweets by Mental Health Category')
plt.xlabel('Year')
...
df_train = pd.read_csv("../Dataset/final_train_data_esp.csv", encoding="utf-8")
df_test = pd.read_csv("../Dataset/final_test_data_esp.csv", encoding = "utf-8")
...
plt.figure(figsize=(12, 8))
df_grouped.plot(kind='bar', stacked=True)
plt.title('Year-wise Distribution of Spanish Tweets by Mental Health Category')
plt.xlabel('Year')
plt.ylabel('Number of Tweets')
plt.legend(title='Mental Health Category')
plt.show()
